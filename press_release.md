...
Every NBA game comes with endless stats, but one question matters most: who is going to win? This project turns complex basketball data into a simple, practical insight by using recent team performance to estimate the probability that a team will win its next game.
...
A logistic regression model was then trained to estimate the probability that the home team wins. The model achieved approximately **62.4% accuracy**, showing that recent performance does provide useful predictive signal. The results also highlight that factors such as shooting efficiency and turnovers play an important role in determining outcomes.

This approach provides a simple and interpretable way for analysts, coaches, and fans to understand how recent team trends translate into win probability.

## Chart

![Win Probability Chart](./docs/win_probability_chart.png)

The chart above shows how a team’s recent scoring advantage affects both predicted and actual win outcomes.
...
As the home team’s scoring advantage increases, both the predicted probability and actual win rate increase. However, the spread of points shows that outcomes are not guaranteed, even when one team appears stronger.
